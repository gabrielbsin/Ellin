...
function doLoserMap(mode, type, selection) {
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            cm.sendNext("Sorry for the loss or tie ... Yet, you have put up quite an good show! Your team work and battle skills were amazing! I'll be expecting the same effort next time!\r\n\r\nMonster Carnival Rank: #b" + letterGrade + "#k");
        } else if (status == 2) {
            var points = cm.getPlayer().getMCPQParty().getTotalCP();
            var grade = getGrade(points);
...
function doWinnerMap(mode, type, selection) {
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            var grade = getGrade(points);
            var letterGrade = "ABCDFEE"[grade];
            cm.sendNext("Congratulations on your victory!!! That was an amazing performance! The opposing party couldn't do a thing! I'll be expecting the same effort next time!\r\n\r\nMonster Carnival Rank: #b" + letterGrade + "\r\n\r\n#k");
...
function doTown(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (selection == 0) {
                if (cm.getPlayer().getLevel() < MonsterCarnival.MIN_LEVEL || cm.getPlayer().getLevel() > MonsterCarnival.MAX_LEVEL) {
